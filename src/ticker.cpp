...
namespace ticker
{
    ...
    Ticker::Ticker(std::shared_ptr<grab::connection::Connection> connection, const std::string& ticker_symbol, const std::string& stream_type)
        : connection_(std::move(connection))  // Store the shared connection (transfer ownership)
        , ticker_symbol_(ticker_symbol)       // Store the pair symbol
        ...
        , is_subscribed_(false)               // Initially not subscribed to any stream
    {
        ...
        this->stream_name_ = this->ticker_symbol_ + "@" + this->stream_type_;
    }

    // Destructor: Ensure clean shutdown by unsubscribing from the stream
    Ticker::~Ticker()
    {
        this->unsubscribe(); // This will unsubscribe and clean up the ticker
    }

    // Set the callback function for handling incoming ticker messages
    void Ticker::set_message_callback(MessageCallback on_message)
    {
    ...
    void Ticker::set_error_callback(ErrorCallback on_error)
    {
    ...
    // Subscribe to the ticker's data stream
    void Ticker::subscribe()
    {
    ...
    // Unsubscribe to the ticker's data stream
    void Ticker::unsubscribe()
    {
        ...
        try
        {
            ...
            this->connection_->unsubscribe_stream(this->ticker_symbol_, this->stream_name_);
        ...
        catch (const std::exception &e)
        {
            ...
            if (on_error_)
            {
                this->on_error_("Error during unsubscribe: " + std::string(e.what()));
            }
    ...
    bool Ticker::is_subscribed() const
    {
        ...
        return this->is_subscribed_ && this->connection_ && this->connection_->is_connected();
    }
