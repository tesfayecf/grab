...
public:
     ...
     * 
     * Controls whether the connection should automatically attempt to
     * reconnect when the connection is lost. Useful for production
     * systems that need continuous data streaming.
     */
...
private:
     ...
     * 
     * Constructs and sends a single subscription message containing all
     * currently registered stream names. Used when the connection is first
     * established to subscribe to all previously registered streams.
     */
     ...
     * 
     * Legacy method that may be used for resubscribing to all streams
     * after a reconnection. Ensures continuity of data streams across
     ...
     * 
     * Constructs a URL for Binance's combined stream format that includes
     * multiple streams in a single connection. This is an alternative to
     * individual subscription messages but is currently not used in favor
     * of the subscription message approach.
     */
    std::string build_stream_url() const;

private:
    // Core components - fundamental objects needed for WebSocket operations
    ...
    // SSL context for secure connections - manages TLS certificates and verification
    ...
    // Buffer for receiving incoming messages - managed by Boost.Beast
    beast::flat_buffer buffer_;

    // Connection state - variables that track the current connection status
    ...
    // Stream management - handles subscription callbacks and stream routing
    ...
    // Timers - manage various timeout and periodic operations
    ...
    // Callbacks - user-defined functions for handling connection events
    ...
    // Connection tracking - timestamps for monitoring connection health
